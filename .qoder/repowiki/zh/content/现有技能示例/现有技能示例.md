...
- [frontend-dev-prompt-craft/results.tsv](file://plugins/frontend-team-toolkit/skills/frontend-dev-prompt-craft/results.tsv)
</cite>
...
F --> F2["references/prompt-templates.md"]
F --> F3["LEARNINGS.md"]
F --> F4["skill-issues.jsonl"]
```
...
- 使用示例
  ...
  - UI 还原：基于设计稿的页面还原任务
  - 接口对接：API 接口文档对接任务
  - **最新学习案例**：异地推荐字段条件渲染提示词生成，涉及后端新增互斥字段（`refereeId` vs `remoteRefereeId`），前端根据字段有无值动态切换 label 和数据绑定，并在异地推荐场景下清空育成人/渠道经理展示
- 配置说明
  - 输入契约：按任务类型定义的必填信息清单
  - 输出契约：完整的提示词格式规范
  - 评估与升级：结构化 eval：`evals/evals.json`、快速实测：`test-prompts.json`、结果记录：`results.tsv`、问题池：`skill-issues.jsonl`（追加新问题）、经验沉淀：`LEARNINGS.md`（追加使用心得）
- 扩展指导
...
**章节来源**
- [frontend-dev-prompt-craft/SKILL.md:11-200](file://plugins/frontend-team-toolkit/skills/frontend-dev-prompt-craft/SKILL.md#L11-L200)
- [frontend-dev-prompt-craft/LEARNINGS.md:1-28](file://plugins/frontend-team-toolkit/skills/frontend-dev-prompt-craft/LEARNINGS.md#L1-L28)
- [frontend-dev-prompt-craft/skill-issues.jsonl:1-4](file://plugins/frontend-team-toolkit/skills/frontend-dev-prompt-craft/skill-issues.jsonl#L1-L4)
...
- [frontend-dev-prompt-craft/results.tsv:1-16](file://plugins/frontend-team-toolkit/skills/frontend-dev-prompt-craft/results.tsv#L1-L16)
...
F --> PT["references/prompt-templates.md"]
F --> LT["LEARNINGS.md"]
